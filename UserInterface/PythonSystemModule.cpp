...
PyObject* systemSetWindowed(PyObject* poSelf, PyObject* poArgs)
{
	...
	CPythonSystem::TConfig* pkConfig = CPythonSystem::Instance().GetConfig();
	pkConfig->bWindowed = iFlag ? true : false;

	if (!pkConfig->bWindowed)
	{
		pkConfig->width = GetSystemMetrics(SM_CXSCREEN);
		pkConfig->height = GetSystemMetrics(SM_CYSCREEN);
	}
...
PyObject* systemGetMSAALevel(PyObject* poSelf, PyObject* poArgs)
{
	return Py_BuildValue("i", CPythonSystem::Instance().GetConfig()->bMSAALevel);
}

PyObject* systemSetMSAALevel(PyObject* poSelf, PyObject* poArgs)
{
	int level;
	if (!PyTuple_GetInteger(poArgs, 0, &level))
		return Py_BuildException();
	...
	CPythonSystem::Instance().GetConfig()->bMSAALevel = (BYTE)level;
	return Py_BuildNone();
}
...
void initsystem()
{
	static PyMethodDef s_methods[] =
	{
		...
		{ "GetMSAALevel", systemGetMSAALevel, METH_VARARGS },
		{ "SetMSAALevel", systemSetMSAALevel, METH_VARARGS },
