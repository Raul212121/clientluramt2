...
void CInstanceBase::__AttachWeaponShiningEffect(int effectIndex, const char* effectFileName, const char* boneName)
{
	...
	if (!strcmp(boneName, "PART_WEAPON"))
	{
		...
		bool bFound = m_GraphicThingInstance.GetAttachingBoneName(
			CRaceData::PART_WEAPON,
			&c_szRightBoneName
		);

		if (bFound && c_szRightBoneName && strcmp(c_szRightBoneName, ""))
		{
			m_weaponShiningEffects[0][effectIndex] =
				m_GraphicThingInstance.AttachEffectByName(
					0,
					c_szRightBoneName,
					effectFileName
				);
	...
	else if (!strcmp(boneName, "PART_WEAPON_LEFT"))
	{
		...
		if (bFound && c_szLeftBoneName && strcmp(c_szLeftBoneName, ""))
		{
			m_weaponShiningEffects[1][effectIndex] =
				m_GraphicThingInstance.AttachEffectByName(
					0,
					c_szLeftBoneName,
					effectFileName
	...
	{
		Tracef(
			"Invalid partname for getting attaching bone name. %s - %s",
			effectFileName,
			boneName
		);
	}
