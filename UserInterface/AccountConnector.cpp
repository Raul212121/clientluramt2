...
bool CAccountConnector::__AuthState_RecvPhase()
{
	...
	else if (kPacketPhase.phase == PHASE_AUTH)
	{
		...
		_snprintf(LoginPacket.securityKey, sizeof(LoginPacket.securityKey), "%s|%s", EXTRA_LOGIN_SECURITY_KEY, CLIENT_VERSION);
		LoginPacket.securityKey[sizeof(LoginPacket.securityKey) - 1] = '\0';
#endif
