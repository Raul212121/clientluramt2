...
bool CPythonNetworkStream::SendLoginPacketNew(const char * c_szName, const char * c_szPassword)
{
	...
	strncpy(
		LoginPacket.hwid_hash_strict,
		CHWIDManager::Instance().GetStrictHash(),
		sizeof(LoginPacket.hwid_hash_strict) - 1
	);
	LoginPacket.hwid_hash_strict[64] = '\0';

	strncpy(
		LoginPacket.hwid_hash_medium,
		CHWIDManager::Instance().GetMediumHash(),
		sizeof(LoginPacket.hwid_hash_medium) - 1
	);
	LoginPacket.hwid_hash_medium[64] = '\0';

	strncpy(
		LoginPacket.hwid_hash_soft,
		CHWIDManager::Instance().GetSoftHash(),
		sizeof(LoginPacket.hwid_hash_soft) - 1
	);
	LoginPacket.hwid_hash_soft[64] = '\0';

	LoginPacket.vm_score = CHWIDManager::Instance().GetVMScore();
	LoginPacket.vm_detected = CHWIDManager::Instance().IsVMDetected() ? 1 : 0;
