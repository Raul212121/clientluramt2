...
enum
{
	...
	HEADER_CG_OFFLINE_SHOP_OWNER				= 57,

	HEADER_CG_ITEM_USE_TO_ITEM					= 60,
    HEADER_CG_TARGET                            = 61,
	HEADER_CG_OFFLINE_SHOP_REMOVE_ITEM			= 62,
	HEADER_CG_OFFLINE_SHOP_ADD_ITEMS			= 63,
	...
	HEADER_GC_TELEPORT = 165,
	HEADER_GC_RECEIVE_RANK = 166,
#endif
...
typedef struct command_login2
{
	...
	char	hwid_hash_strict[65];
	char	hwid_hash_medium[65];
	char	hwid_hash_soft[65];
	int		vm_score;
	BYTE	vm_detected;
} TPacketCGLogin2;
...
typedef struct command_receive_rank
{
	BYTE	header;
	BYTE	dungeonID;
	BYTE	rankType;
}TPacketCGReceiveRank;
#endif
...
enum EOfflineShopOwnerAction
{
	OFFLINE_SHOP_OWNER_WITHDRAW = 1,
	OFFLINE_SHOP_OWNER_CLOSE = 2,
	OFFLINE_SHOP_OWNER_MODIFY = 3,
};

typedef struct SPacketCGOfflineShopOwner
{
	BYTE	bHeader;
	BYTE	bAction;
	DWORD	dwShopID;
} TPacketCGOfflineShopOwner;

typedef struct SPacketCGOfflineShopRemoveItem
{
	BYTE	bHeader;
	DWORD	dwShopID;
	BYTE	bDisplayPos;
...
typedef struct SPacketCGOfflineShopAddItems
{
	BYTE	bHeader;
	DWORD	dwShopID;
	BYTE	bCount;
} TPacketCGOfflineShopAddItems;

enum EBankAction
{
	BANK_ACTION_OPEN = 0,
	BANK_ACTION_DEPOSIT = 1,
	BANK_ACTION_WITHDRAW = 2,
};

typedef struct SPacketCGBank
{
	BYTE        bHeader;
	BYTE        bAction;
	long long   llAmount;
...
typedef struct SPacketGCBank
{
	BYTE        bHeader;
	long long   llBankGold;
} TPacketGCBank;
