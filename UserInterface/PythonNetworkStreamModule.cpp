...
PyObject* netSendOfflineShopOwnerPacket(PyObject* poSelf, PyObject* poArgs)
{
	int iAction;
	...
	int iShopID;
	if (!PyTuple_GetInteger(poArgs, 1, &iShopID))
		return Py_BuildException();

	CPythonNetworkStream& rkNetStream = CPythonNetworkStream::Instance();
	rkNetStream.SendOfflineShopOwnerPacket(
		(BYTE)iAction,
		(DWORD)iShopID
	);
...
PyObject* netSendOfflineShopRemoveItemPacket(
	PyObject* poSelf,
	PyObject* poArgs)
{
	int iShopID;

	if (!PyTuple_GetInteger(
		poArgs,
		0,
		&iShopID))
	{
	...
	CPythonNetworkStream& rkNetStream =
		CPythonNetworkStream::Instance();

	rkNetStream.SendOfflineShopRemoveItemPacket(
		(DWORD)iShopID,
		(BYTE)iDisplayPos
	);
...
PyObject* netSendOfflineShopAddItemsPacket(
	PyObject* poSelf,
	PyObject* poArgs)
{
	int iShopID;
	...
	std::vector<TOfflineShopAddItem> items;
	...
	for (Py_ssize_t i = 0; i < itemCount; ++i)
	{
		...
		if (!pyItem ||
			!PyTuple_Check(pyItem) ||
			PyTuple_Size(pyItem) != 4)
		{
			return Py_BuildException();
		}
		...
		if (!PyTuple_GetInteger(pyItem, 0, &iWindowType))
			return Py_BuildException();

		if (!PyTuple_GetInteger(pyItem, 1, &iCell))
			return Py_BuildException();

		if (!PyTuple_GetInteger(pyItem, 2, &iPrice))
			return Py_BuildException();

		if (!PyTuple_GetInteger(pyItem, 3, &iDisplayPos))
			return Py_BuildException();

		TOfflineShopAddItem item;

		item.pos.window_type = (BYTE)iWindowType;
		item.pos.cell = (WORD)iCell;
		item.price = (DWORD)iPrice;
		item.displayPos = (BYTE)iDisplayPos;
	...
	rkNetStream.SendOfflineShopAddItemsPacket(
		(DWORD)iShopID,
		items
	);
...
PyObject* netSendBankPacket(
	PyObject* poSelf,
	PyObject* poArgs)
{
	int iAction;
	PY_LONG_LONG llAmount;
...
PyObject* netDungeonRank(PyObject* poSelf, PyObject* poArgs)
{
	int dungeonID;
	if (!PyTuple_GetInteger(poArgs, 0, &dungeonID))
		return Py_BuildException();

	int rankType;
	if (!PyTuple_GetInteger(poArgs, 1, &rankType))
		return Py_BuildException();

	CPythonNetworkStream& rkNetStream = CPythonNetworkStream::Instance();
	rkNetStream.DungeonRank(dungeonID, rankType);
	return Py_BuildNone();
}
#endif
...
PyObject* netSendChestDropInfo(PyObject* poSelf, PyObject* poArgs)
{
	...
	CPythonNetworkStream& rkNetStream = CPythonNetworkStream::Instance();
	rkNetStream.SendChestDropInfo(TItemPos(iWindowType, iCell));
...
void initnet()
{
	static PyMethodDef s_methods[] =
	{
		...
		{ "SendOfflineShopOwnerPacket",			netSendOfflineShopOwnerPacket,			METH_VARARGS },
		{ "SendOfflineShopRemoveItemPacket",	netSendOfflineShopRemoveItemPacket,		METH_VARARGS },
		{ "SendOfflineShopAddItemsPacket",		netSendOfflineShopAddItemsPacket,		METH_VARARGS },
		{ "SendBankPacket",                     netSendBankPacket,						METH_VARARGS },
		...
		{ "DungeonRank",								netDungeonRank,							METH_VARARGS },
#endif
		{ NULL,										NULL,										NULL },
	};
