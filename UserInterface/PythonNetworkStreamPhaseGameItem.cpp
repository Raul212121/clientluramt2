...
bool CPythonNetworkStream::SendShopSellPacketNew(WORD bySlot, WORD byCount)
{
	if (!__CanActMainInstance())
		return true;
...
bool CPythonNetworkStream::DungeonRank(DWORD dungeonID, DWORD rankType)
{
	if (!__CanActMainInstance())
		return true;

	TPacketCGReceiveRank p;
	p.header = HEADER_GC_RECEIVE_RANK;
	p.dungeonID = dungeonID;
	p.rankType = rankType;
	if (!Send(sizeof(TPacketCGReceiveRank), &p))
	{
		Tracen("DungeonRank Error");
		return false;
	}
