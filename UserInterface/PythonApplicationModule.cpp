...
static CMobPreviewTest g_MobPreviewTest;
static const int WIKI_MOB_PREVIEW_COUNT = 4;
static CMobPreviewTest g_WikiMobPreview[WIKI_MOB_PREVIEW_COUNT];
...
PyObject* appUpdateGame(PyObject* poSelf, PyObject* poArgs)
{
	...
	g_MobPreviewTest.Update();

	for (int i = 0; i < WIKI_MOB_PREVIEW_COUNT; ++i)
		g_WikiMobPreview[i].Update();
...
PyObject* appCreateMobPreview(PyObject* poSelf, PyObject* poArgs)
{
	int iVnum;
	...
	bool bResult = g_MobPreviewTest.Create((DWORD)iVnum);

	return Py_BuildValue("i", bResult ? 1 : 0);
}

PyObject* appToggleMobPreview(PyObject* poSelf, PyObject* poArgs)
{
	g_MobPreviewTest.Toggle();

	return Py_BuildNone();
}

PyObject* appSetMobPreviewViewport(PyObject* poSelf, PyObject* poArgs)
{
	int x;
	int y;
	int width;
	...
	g_MobPreviewTest.SetViewport(x, y, width, height);

	return Py_BuildNone();
}

PyObject* appRenderMobPreview(PyObject* poSelf, PyObject* poArgs)
{
	g_MobPreviewTest.Render();

	return Py_BuildNone();
}

PyObject* appCreateWikiMobPreview(PyObject* poSelf, PyObject* poArgs)
{
	int iIndex;
	int iVnum;
	...
	if (iIndex < 0 || iIndex >= WIKI_MOB_PREVIEW_COUNT)
		return Py_BuildValue("i", 0);

	bool bResult = g_WikiMobPreview[iIndex].Create((DWORD)iVnum);

	return Py_BuildValue("i", bResult ? 1 : 0);
}

PyObject* appSetWikiMobPreviewViewport(PyObject* poSelf, PyObject* poArgs)
{
	int iIndex;
	int x;
	int y;
	...
	if (!PyTuple_GetInteger(poArgs, 3, &width))
		return Py_BuildException();

	if (!PyTuple_GetInteger(poArgs, 4, &height))
		return Py_BuildException();

	if (iIndex < 0 || iIndex >= WIKI_MOB_PREVIEW_COUNT)
		return Py_BuildNone();

	g_WikiMobPreview[iIndex].SetViewport(
		x,
		y,
		width,
		height
...
PyObject* appRenderWikiMobPreview(PyObject* poSelf, PyObject* poArgs)
{
	int iIndex;
	...
	g_WikiMobPreview[iIndex].Render();
...
void initapp()
{
	...
	{	
		...
		{ "CreateMobPreview",			appCreateMobPreview,			METH_VARARGS },
		{ "ToggleMobPreview",			appToggleMobPreview,			METH_VARARGS },
		{ "RenderMobPreview",			appRenderMobPreview,			METH_VARARGS },
		{ "SetMobPreviewViewport",		appSetMobPreviewViewport,		METH_VARARGS },
		{ "CreateWikiMobPreview", appCreateWikiMobPreview,				METH_VARARGS },
		{ "SetWikiMobPreviewViewport", appSetWikiMobPreviewViewport,	METH_VARARGS },
		{ "RenderWikiMobPreview", appRenderWikiMobPreview, METH_VARARGS },
