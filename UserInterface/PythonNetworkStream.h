...
class CPythonNetworkStream : public CNetworkStream, public CSingleton<CPythonNetworkStream>
{
	...
	public:
		...
		bool DungeonTeleport(DWORD coordX, DWORD coordY);
		bool DungeonRank(DWORD dungeonID, DWORD rankType);
#endif
		bool RecvBiologInfo();
		...
		bool SendShopSellPacketNew(WORD bySlot, WORD byCount);
		...
		bool SendOfflineShopOwnerPacket(BYTE bAction, DWORD dwShopID);
		bool SendOfflineShopRemoveItemPacket(
			DWORD dwShopID,
			BYTE bDisplayPos
		);

		bool SendOfflineShopAddItemsPacket(
			DWORD dwShopID,
			const std::vector<TOfflineShopAddItem>& items
		);
		// Bank
		bool SendBankPacket(BYTE bAction, long long llAmount);
		// Refine
