...
bool CPythonNetworkStream::RecvWhisperPacket()
{
	...
	if (CPythonChat::WHISPER_TYPE_CHAT == whisperPacket.bType || CPythonChat::WHISPER_TYPE_GM == whisperPacket.bType)
	{
		...
		if (hWnd && GetForegroundWindow() != hWnd)
		{
			...
			ZeroMemory(&flashInfo, sizeof(flashInfo));
...
bool CPythonNetworkStream::SendBuildOfflineShopPacket(
	const char* c_szName,
	const std::vector<TShopItemTable>& c_rSellingItemStock)
{
	...
	strncpy(packet.szSign, c_szName, SHOP_SIGN_MAX_LEN);
	packet.szSign[SHOP_SIGN_MAX_LEN] = '\0';

	packet.bCount = static_cast<unsigned char>(
		c_rSellingItemStock.size()
		);
...
bool CPythonNetworkStream::SendOfflineShopOwnerPacket(
	BYTE bAction,
	DWORD dwShopID)
{
	TPacketCGOfflineShopOwner packet;

	packet.bHeader = HEADER_CG_OFFLINE_SHOP_OWNER;
	...
	{
		TraceError(
			"SendOfflineShopOwnerPacket failed action=%u shop=%u",
			(DWORD)bAction,
			dwShopID
		);
...
bool CPythonNetworkStream::SendOfflineShopRemoveItemPacket(
	DWORD dwShopID,
	BYTE bDisplayPos)
{
	TPacketCGOfflineShopRemoveItem packet;

	packet.bHeader = HEADER_CG_OFFLINE_SHOP_REMOVE_ITEM;
	packet.dwShopID = dwShopID;
	packet.bDisplayPos = bDisplayPos;

	if (!Send(sizeof(packet), &packet))
	{
		TraceError(
			"SendOfflineShopRemoveItemPacket failed shop=%u pos=%u",
			dwShopID,
			(DWORD)bDisplayPos
		);
...
bool CPythonNetworkStream::SendOfflineShopAddItemsPacket(
	DWORD dwShopID,
	const std::vector<TOfflineShopAddItem>& items)
{
	if (items.empty())
		return false;

	if (items.size() > SHOP_HOST_ITEM_MAX_NUM)
		return false;

	TPacketCGOfflineShopAddItems packet;

	packet.bHeader = HEADER_CG_OFFLINE_SHOP_ADD_ITEMS;
	packet.dwShopID = dwShopID;
	packet.bCount = (BYTE)items.size();

	if (!Send(sizeof(packet), &packet))
	{
		TraceError(
			"SendOfflineShopAddItemsPacket header failed shop=%u count=%u",
			dwShopID,
			(DWORD)packet.bCount
		);
	...
	for (std::vector<TOfflineShopAddItem>::const_iterator it =
		items.begin();
		it != items.end();
		++it)
	{
		if (!Send(sizeof(TOfflineShopAddItem), &(*it)))
		{
			TraceError(
				"SendOfflineShopAddItemsPacket item failed shop=%u",
				dwShopID
			);
...
bool CPythonNetworkStream::SendBankPacket(
	BYTE bAction,
	long long llAmount)
{
	TPacketCGBank packet;

	packet.bHeader = HEADER_CG_BANK;
	packet.bAction = bAction;
	packet.llAmount = llAmount;

	if (!Send(sizeof(packet), &packet))
	{
		TraceError(
			"SendBankPacket failed action=%u amount=%lld",
			(DWORD)bAction,
			llAmount
		);
...
{
	TPacketGCBank packet;

	if (!Recv(sizeof(packet), &packet))
	{
		TraceError("RecvBankPacket failed");
		return false;
	}
	...
	PyCallClassMemberFunc(
		...
		"BINARY_BankUpdate",
		Py_BuildValue("(L)", packet.llBankGold)
	);
...
bool CPythonNetworkStream::RecvBattlePassPacket()
{
	...
	while (packet.wSize > 0)
	{
		...
		PyCallClassMemberFunc(
			m_apoPhaseWnd[PHASE_WINDOW_GAME],
			"BINARY_BattlePassAddMission",
			Py_BuildValue(
				"(iiiii)",
				missionInfo.dwMissionId,
				missionInfo.bMissionType,
				missionInfo.dwMissionInfo[0],
				missionInfo.dwMissionInfo[1],
				missionInfo.dwMissionInfo[2]
			)
		);
...
bool CPythonNetworkStream::RecvBattlePassUpdatePacket()
{
	...
	PyCallClassMemberFunc(
		m_apoPhaseWnd[PHASE_WINDOW_GAME],
		"BINARY_BattlePassUpdate",
		Py_BuildValue(
			"(iii)",
			packet.dwMissionId,
			packet.bMissionType,
			packet.dwNewProgress
		)
	);
