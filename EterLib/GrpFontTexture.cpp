...
bool CGraphicFontTexture::Create(const char* c_szFontName, int fontSize, bool bItalic)
{
	...
	if (GetMaxTextureWidth() >= 2048)
		width = 2048;
	else if (GetMaxTextureWidth() >= 1024)
		width = 1024;
	else
		width = 512;

	if (GetMaxTextureHeight() >= 2048)
		height = 2048;
	else if (GetMaxTextureHeight() >= 512)
		height = 1024;
	else
		height = 512;
...
bool CGraphicFontTexture::AppendTexture()
{
	...
	if (!pNewTexture->Create(m_dib.GetWidth(), m_dib.GetHeight(), D3DFMT_A8R8G8B8))
	{
		delete pNewTexture;
		return false;
	}
...
CGraphicFontTexture::TCharacterInfomation* CGraphicFontTexture::UpdateCharacterInfomation(TCharacterKey code)
{
	...
	for (nChrY = 0; nChrY < nChrHeight; ++nChrY, pdwDIBRow += nDIBWidth)
	{
		...
		{
			DWORD src = pdwDIBRow[nChrX] & 0xFF;

			if (src == 0)
			{
				pdwDIBRow[nChrX] = 0x00000000;
				continue;
			}

			BYTE a = (BYTE)src;
			pdwDIBRow[nChrX] = (a << 24) | 0x00FFFFFF;
		}
	}
